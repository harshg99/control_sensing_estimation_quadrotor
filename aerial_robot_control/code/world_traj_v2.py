...
class WorldTraj(object):
    ...
    def __init__(self, world, start, goal):
        ...
        for j in range(self.unit_vectors.shape[0]-1):
            dotp = np.dot(self.unit_vectors[j+1],self.unit_vectors[j])
            if dotp<=0.4:
                # if self.distances[j+1]<1.5:
                #     self.spline_times[j] = 1.1*self.spline_times[j]
                self.spline_times[j+1] = (1.8-2*dotp)*self.spline_times[j+1]
                self.spline_times[j] = (1.4-dotp)*self.spline_times[j]

        if self.distances[-1]<2:
            self.spline_times[-1] = self.spline_times[-1]*1.2
        else:
            self.spline_times[-1] = self.spline_times[-1]*1.2
