...
class WorldTraj(object):
    ...
    def __init__(self, world, start, goal):
        ...
        self.resolution = np.array([0.10, 0.10, 0.10])
        self.margin = 0.60
        ...
        for j in range(self.points.shape[0]-1):
            ...
            if j==0:
                self.unit_vectors[j] = np.array([0,0,0])
        ...
        for j in range(self.points.shape[0]-1):
            ...
            #     if self.distances[j]>=9.0 and np.dot(self.unit_vectors[j],self.unit_vectors[j-1])>=0.3:                    
            #         velocity_min = self.velocity_min*1.35
            #     elif self.distances[j]>=4.5 and np.dot(self.unit_vectors[j],self.unit_vectors[j-1])>=0.3:                    
            #         velocity_min = self.velocity_min*1.15
            #     elif self.distances[j]<=0.5:
            #         velocity_min = self.velocity_min/1.00
            #     else:
            #         velocity_min = self.velocity_min
            # else:
            #     if self.distances[j]>=2.0:
            #         velocity_min = self.velocity_min/1.0
            #     elif self.distances[j]<=0.5:
            #         velocity_min = self.velocity_min/1.0
            #     else:
            #         velocity_min = self.velocity_min
        ...
        #T()
        self.spline_times[0] += 0.2
        # parameters

        print("Finding spline params")
        self.coeffsx = self.compute_splines(self.points[:,0],self.spline_times)
        self.coeffsx = self.coeffsx.reshape((self.N - 1, -1))
        self.coeffsy= self.compute_splines(self.points[:,1],self.spline_times)
        self.coeffsy = self.coeffsy.reshape((self.N - 1, -1))
        self.coeffsz = self.compute_splines(self.points[:,2],self.spline_times)
    ...
    def simplify_path(self,path):
        ...
        for idx in range(len(waypoints)-1):
            if np.linalg.norm(simplified_waypoints[-1]-waypoints[idx+1])>=0.5:
                simplified_waypoints.append(waypoints[idx+1])
    ...
    def update(self, t):
        ...
        if index >= self.N - 1:
          ...
          # x_dot[0] = self.coeffsx[index, :].T @ vel_t
          # x_dot[1] = self.coeffsy[index, :].T @ vel_t
          # x_dot[2] = self.coeffsz[index, :].T @ vel_t
          
          # x_ddot[0] = self.coeffsx[index, :].T @ acc_t
          # x_ddot[1] = self.coeffsy[index, :].T @ acc_t
          # x_ddot[2] = self.coeffsz[index, :].T @ acc_t
    ...
    def get_acc_matrix(self,time,order):
        arr = np.array([((order - j)**2 - (order - j))*time**(order - j - 2) for j in range(order + 1)])
        #arr [-2:] = 0
        return arr
